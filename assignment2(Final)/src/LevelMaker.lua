...
function LevelMaker.createMap(level)
    keyLevel = math.random(2) == 1 and true or false --Update(III): Level with keyBrick or not(probability 1/2)

    local keyFlag = false --Update(III):Flag for key brick placed or not
    local bricks = {}
    ...
    for y = 1, numRows do
        ...
        for x = 1, numCols do
            ...
            --Update(III):If its key level and key brick is not placed 
            if keyLevel and math.random(3) == 1 and not keyFlag then

                b.color = 5
                b.tier = 5 --hehe just a solution to index 22
                keyFlag = true-- brick is placed
            else
    ...
    -- in the event we didn't generate any bricks
    -- Update(III): or if its keylevel and there is no brick, try again
    if (#bricks == 0) or (keyLevel and not keyFlag)then
